...
app.layout = html.Div([
		...
		html.Div(id='summary',style={'backgroundColor':'#FF0000'}),
...
def youtube(yt_link):
    ...
    summed = html.Div([dcc.Markdown('''
    		...
    		**Highly Likeable:** {}
    	'''.format(v_vid.iloc[0]['title'],l_vid.iloc[0]['title'],d_vid.iloc[0]['title'],
    	r_vid.iloc[0]['title'],lr_vid.iloc[0]['title']))],style={'color':'#FFFFFF','fontSize':10})
    ...
    layout=do.Layout(title=do.layout.Title(text='Like Ratio (Likes / Reaction Count)', x=0.5), margin=margins,
    			polar={'radialaxis':{'showticklabels':False}, 'angularaxis':{'showticklabels':False}}))
    
   
    return view, like, dislike, react, react_ratio, like_ratio, summed
